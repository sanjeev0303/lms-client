...
class AnalyticsBatcher {
     ...
     */
    private async processCourseViews(events: AnalyticsEvent[]): Promise<void> {
        if (events.length === 0) return;
        ...
        const viewsByCourse = events.reduce((views, event) => {
            ...
            views[courseId].totalDuration += (event.data.duration as number) || 0;
            return views;
        }, {} as Record<string, CourseViewData>);
     ...
     */
    private async processLectureCompletions(events: AnalyticsEvent[]): Promise<void> {
        if (events.length === 0) return;
     ...
     */
    private async sendLectureCompletion(lectureId: string, courseId: string, data: unknown): Promise<void> {
        console.log(`[BATCHED] Lecture completion: ${lectureId} in course ${courseId}`, data);
